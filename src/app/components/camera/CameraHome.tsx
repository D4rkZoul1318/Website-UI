...
const BOOT_CHECKS = ['Sensor', 'Lens', 'Storage', 'Stabilisation'];
...
export default function CameraHome() {
  ...
  // and animations (see camera-home.css). Previously this was a GSAP
  // timeline built from a long .set()/.to() chain: if anything threw
  // partway through building it (before .play() was reached), the
  // timeline was left paused forever in whatever partial state its early
  // .set() calls had already applied — reported as the boot screen stuck
  // blurred on a real visit, never reproduced in any automated test. A
  // JS-authored timeline is inherently vulnerable to that: it's a
  // sequence of imperative calls that can be interrupted anywhere in the
  // middle. The fix here isn't a sturdier timeline, it's not having one —
  // `isOpening` is a boolean React renders as present or absent, with no
  // partial state in between, and every actual duration/delay lives in
  // CSS, not in this file.
  //
  // Both effects below are deliberately trivial: no refs, no
  // querySelector, nothing that can return null and abort a chain of
  // setup calls partway through. Toggling a boolean is about as close to
  // impossible-to-partially-fail as JS gets.
  ...
  useEffect(() => {
    // One paint with the pre-opening state first (boot opaque, main
    // blurred), then flip on the next frame — same reasoning the old
    // effect's useLayoutEffect had, just achieved by scheduling the class
    // change a frame late instead of setting inline styles synchronously
    // pre-paint (there's nothing left to flash: the CSS above already
    // authors the "start" state as the default, un-classed one).
    //
    // The setTimeout is a hard fallback, independent of the rAF callback
    // ever running: if the rAF callback throws before calling
    // setIsOpening (simulating the exact "JS dies mid-sequence" failure
    // this rebuild targets), the reveal still happens once the fallback
    // fires — later than intended, but not stuck forever.
    const raf = requestAnimationFrame(() => setIsOpening(true));
    const fallback = window.setTimeout(() => setIsOpening(true), 5000);
    return () => {
  ...
  useEffect(() => {
    ...
    // sequence early. 'wheel' and 'touchstart' are deliberately excluded:
    // scrolling/touching the instant a page loads is close to an
    // involuntary reflex, and including them here previously collapsed
    // the sequence on nearly every real visit before it ever played.
    const skip = () => {
      setNoAnim(true);
      setIsOpening(true);
    };
  ...
  const rootClassName =
    'camera-theme camera-home' + (isOpening ? ' is-opening' : '') + (noAnim ? ' no-anim' : '');
    ...
    <div className={rootClassName}>
      <div id="boot" aria-hidden="true">
        ...
        <div className="boot-checks">
          {BOOT_CHECKS.map((label, i) => (
            <div className="boot-check" style={{ '--i': i } as React.CSSProperties} key={label}>
              <span className="boot-check-glyph">
                <span className="glyph-dash">—</span>
