...
function createProgram(gl: WebGL2RenderingContext, shaderSources: string[], tfv?: any, attribLocs?: Record<string, number>) {
  ...
  if (attribLocs) for (const a in attribLocs) gl.bindAttribLocation(program, attribLocs[a], a);
...
class InfiniteGridMenu {
  ...
  init(onInit: ((s: any) => void) | null) {
    ...
    this.discProgram = createProgram(gl, [discVertShaderSource, discFragShaderSource], null, { aModelPosition: 0, aModelNormal: 1, aModelUvs: 2, aInstanceMatrix: 3 }) as WebGLProgram;
    this.discLocations = {
  ...
  onControlUpdate(deltaTime: number) {
    ...
    let damping = 5/ts, cameraTargetZ = 3*this.scaleFactor;
...
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick }: { items?: MenuItem[]; scale?: number; onItemClick?: (item: MenuItem) => void }) {
  ...
  useEffect(() => {
    ...
    const sketch = new InfiniteGridMenu(
      ...
      index => setActiveItem((items.length ? items : defaultItems)[index % Math.max(1, (items.length || 1))]),
      setIsMoving,
      sk => sk.run(),
      scale
    );
